package com.gubee.springWithKotlin.integration
...
class HttpControllersTests(@Autowired val mockMvc: MockMvc) {
    ...
    fun `List clients`() {
        val johnDoe = Client("johnDoe", "John", "Doe")
        val janeDoe = Client("janeDoe", "Jane", "Doe")
        every { clientRepository.findAll() } returns listOf(johnDoe, janeDoe)
        mockMvc.perform(get("/api/client/").accept(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk)
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
